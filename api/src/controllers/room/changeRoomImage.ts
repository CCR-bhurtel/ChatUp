...
const changeRoomImage = catchAsync(
  async (req: ExpressRequest, res: Response, next: NextFunction) => {
    ...
    const existingRoom = await Room.findById(roomId).populate({
      path: "roomAdmin",
      select: "name",
    });

    if (!existingRoom)
      return next(new AppError("No room found with given id", 400));

    if (
      existingRoom.isGroupChat &&
      existingRoom.roomAdmin?._id.toString() !== req.user._id.toString()
    )
    ...
    try {
      if (
        ...
        !existingRoom.roomImage?.startsWith("default")
      )
        await fs.unlinkSync(
          path.join(
            __dirname,
            "../../../public/images/groupImages",
            existingRoom.roomImage
          )
        );
    } catch (err) {
      logger.error("Error deleting group image", err);
    }
